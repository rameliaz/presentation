...
::: {.notes}
Despite widely cited, Barbour's taxonomy remain silent in at least three following aspects which I addressed as a focus in my dissertation.
...
Two decision diamonds here represent cognitive flexibility and motivational forces required for the activation of explanatory coexistence. If these condition aren't met, people adopt conflict.
...
::: {.notes}
In the second manuscript, we shift the focus from conceptual to operational level. Therefore, this manuscript questions...
...
::: {.smaller}
* There have been efforts to operationalize Barbour's taxonomy by [assigning participants to one]{.mark} of the mental models.
* ...but this [violates]{.mark} Barbour's assumption.
...
::: {.notes}
Barbour proposes taxonomy but he cautions not to use it for categorizing people...
To understand these two assumptions better, I will use an example.
...
::: {.smaller}
* There have been some efforts to operationalize Barbour's taxonomy, but these studies violated Barbour's most important assumption by [assigning participants to one]{.mark} of the mental models.
* One way to operationalize the taxonomy is to assume that mental models do not only differ in their [kind]{.mark} but also in their [degree]{.mark} of conflict ![](resources/arrow.svg){.icon} compatibility.
...
* Since mental models are [culturally embedded]{.mark} [(Johnson, et al., 2020; Rios & Aveyard, 2019; Rios & Roth, 2020)]{.much-smaller}, the scale might contain a systematic cultural bias, so we performed a differential item functioning (DIF) analysis.
...
Convergent validity: compared it with existing scale measuring same construct with different theoretical reasoning
...
::: {.notes}
Taking one study as an example, we found that items reflecting the same mental model were grouped together, where items representing conflict and consonance -- the two extremes --- sit in the opposite direction.
...
* Small-to-moderate correlations between self-identification with atheism (both in ![](resources/de.svg){.icon} and ![](resources/usa.svg){.icon}) and science (only ![](resources/de.svg){.icon}) with [higher conflict]{.mark} perception. 
* Close to zero correlation between identifying as agnostic and perceptions of conflict/compatibility perceptions, both in ![](resources/de.svg){.icon} and ![](resources/usa.svg){.icon} samples.
...
::: {.notes}
We learned from the second manuscript of the project that religious participants tend to see compatibility between sci and rel. However, this contradicts many previous studies demonstrating that religious individuals, even though they tend to see compatibility between science and religion, trust science less than non-religious individuals.
...
::: {.notes}
The dilution effect here implies that sci and rel are seen as substitutes so people would rely less on them. This aligns with a metaphor that "all roads lead to Rome". 
...
* The order of both *incidents* and *number of explanations* was [counterbalanced]{.mark}; each condition only appeared once.
...
- This general perception of the relationship between S and R [only predicts]{.mark} how people evaluate the utility of [religious explanations]{.mark}, and *not* scientific explanations.
:::
...
![](img/pillar2/interrater_q.png)

#### *N* = 8, ICC2*k* = 0.94, 95% CI [0.90, 0.97], *F*(26, 182) = 18, *p* < .001
