...
* [Assumptions]{.mark}: one cannot fit people neatly into one *taxon* and these are situation-dependent characteristics [(Barbour, 2002)]{.much-smaller}.
* Perceptions of the relationship between S and R are defined as a [mental schema]{.mark} for [processing potentially competing S and R explanations]{.mark}, especially in situations in which both scientific and religious explanations are salient. 
...
::: {.notes}
This is a conceptual paper, exploring the first question.
...
* Explanatory coexistence can occur because S and R have [differential utility]{.mark} [(Davoodi & Lombrozo, 2022)]{.much-smaller} and thus are [processed differently]{.mark} [(Van Leeuwen, 2014)]{.much-smaller}.
...
::: {.notes}
People give different labels to the same mental model. For example, "independence" is labelled as "compartment" or "NOMA".
...
We illustrate how cognitive and motivational elements determine how people adopt these mental models in a simplified version of the flowchart we presented in the article.
...
::: {.smaller}
* There have been some efforts to operationalize Barbour's taxonomy, but violating Barbour's assumption by [assigning participants to one]{.mark} of the mental models.
...
* The construct was [conceptually distinct]{.mark} from Belief in Science [(Farias et al., 2013)]{.much-smaller} ($\rho^2$ = .46 ![](resources/de.svg){.icon}, .25 ![](resources/usa.svg){.icon}) and [similar to]{.mark} Leicht et al.'s [(2021)]{.much-smaller} scale ($\rho^2$ = .57 ![](resources/de.svg){.icon} .54 ![](resources/usa.svg){.icon}), but sufficient unshared variance still left.
...
[@kruglanskiHowManyRoads2011; @jacksonReligiousPeopleView2024] [@kundaCaseMotivatedReasoning1990] 
