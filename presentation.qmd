...
* Therefore, S and R are [processed differently]{.mark} [(Van Leeuwen, 2014)]{.much-smaller}.
...
Discriminant: we compare it with belief in science
Convergent validity: compare it with another scale measuring same construct with different theoretical reasoning
...
::: {.notes}
As an example, in one study we found that items reflecting the same mental model were grouped together, where items representing conflict and consonance -- the two extremes --- sit in the opposite direction.
...
* The construct was [conceptually distinct]{.mark} from Belief in Science [(Farias et al., 2013)]{.much-smaller} ($\rho^2$ = .46 ![](resources/de.svg){.icon}, .25 ![](resources/usa.svg){.icon}) and [similar to]{.mark} Leicht et al.'s [(2021)]{.much-smaller} scale ($\rho^2$ = .57 ![](resources/de.svg){.icon}, .54 ![](resources/usa.svg){.icon}), but [sufficient unshared variance]{.mark} still left.
...
* Small-to-moderate correlations between atheist identity (both in ![](resources/de.svg){.icon} and ![](resources/usa.svg){.icon}) and science identity (only ![](resources/de.svg){.icon}) with [higher conflict]{.mark} perception. 
...
::: {.notes}
The dilution effect here implies that sci and rel are seen as substitutes so people would rely less on them. This aligns with a metaphor: "all roads lead to Rome". 
...
::: {.notes}
Thank you so much for your attention! I would welcome all questions that have.
